...
| --- | --- | --- |
| [desktop.mjs](desktop.mjs) | `npm run demo`, `npm run build:app` | Tauri in apps/desktop; builds under apps/desktop/src-tauri/target |
| [rust.mjs](rust.mjs) | `npm run test:rust` | Cargo in apps/desktop/src-tauri; forwards CLI arguments |
| [generate-contracts.mjs](generate-contracts.mjs) | `npm run contracts:generate` | Runs generate-contracts.ts with installed Vite/Zod and console aliases; writes docs/contracts/*.schema.json and fixtures/alice/*.json |

[paths.mjs](paths.mjs) owns checkout discovery. `npm run test:scripts` verifies
...
See the [console guide](../../docs/guides/technician-console.md).
