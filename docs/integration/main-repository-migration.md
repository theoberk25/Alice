...
The technician console now uses the shared top-level app, package and service layout.
The original import into workstation is historical; the [layout migration](../handoffs/2026-09-05-console-layout.md) records its replacement.
...
Use the [main workstation guide](../guides/workstation.md) for setup and
current migration verification. Commands and code paths in the subsystem docs
are relative to the repository root unless they explicitly say otherwise.
...
| Original destination (historical) | `workstation/`; the pre-existing tracked workstation placeholders were preserved, with no path collisions |
| Current dependency boundary | Root npm workspace/lock, apps/desktop/src-tauri Cargo manifest/lock, and services/biometrics requirements/environment |
...
The separate [historical verification record](../guides/console/verification.md)
...
| --- | --- | --- |
| `apps/desktop/` | `apps/desktop/` | React/Vite UI and Tauri/Rust native boundary |
| `packages/contracts/` | `packages/contracts/` | Executable console Zod schemas and legacy adapter |
| `packages/domain/` | `packages/domain/` | Review state, immutable lineage and interface contracts |
| `packages/ui/` | `packages/ui/` | Shared console presentation components |
| `services/biometrics/` | `services/biometrics/` | Local facial identity service and tests |
| `fixtures/` | `fixtures/` | Byte-preserved original dashboard contract, extracted/normalized fixtures and deterministic scenarios |
| `tests/`, `scripts/` | `tests/console/`, `scripts/console/`, `scripts/biometrics/` | Console checks, launchers and provisioning helpers |
| `docs/`, root Markdown | `docs/architecture/`, `docs/contracts/`, `docs/guides/`, `docs/integration/`, `docs/handoffs/` | Console implementation/reference docs, adapted setup and historical evidence |
| Root manifests, locks and configuration | Repository root | Independent subsystem tooling and private `.env` boundary |

The npm workspace now lives at the repository root. Core `common/`,
...
The main [architecture](../prds/ALICE-DCAMR-Architecture.md),
[product PRD](../prds/ALICE-DCAMR-PRD.md),
[team handoff](../prds/ALICE-DCAMR-PRD-Handoff.md), and
[console integration requirements](technician-console.md)
...
These remain console-owned in apps/desktop, packages and services/biometrics: native IPC and configuration, administrator and
