...
| 113 | Run Technician Approval Test | Partial | User now confirms human approval turned on physical light; detailed action-specific ledger reconciliation remains. Enterprise ingress at Mac :8790 verified Wazuh-first receipt → Pi HOLD and exact replay; added immutable USB enterprise receipt cache via authenticated SSH, checked before forwarding; seven tests passed. Wireless laptop confirmation pending. 2026-09-06: signed generation 4 with existing matching manifest key, enabling approval_required on G-LIGHT-ON only; both grants, subjects and terminal keys preserved. Backup `/mnt/alice-usb/release-before-merek-review-004`. Live `merek-live-hold-004`: HTTP202 CHALLENGE/PERMISSION_REVIEW_REQUIRED; review eligible READY/PENDING/NOT_EXECUTED. Awaiting Merek human disposition; permission-triggered, not ML inference. 2026-09-06 live probe: `merek-live-allow-071902` auto-ALLOW, physical controller COMPLETED/ON, USB ledger sequences 630–636; review correctly ineligible (`MACHINE_DECISION_NOT_REVIEWABLE`). Invalid target request rejected at schema validation, not technician/policy denial. Wazuh delivery TRANSPORT_UNAVAILABLE/retrying. User confirmed Merek sees ALLOW/COMPLETED/ON and disabled review controls; human disposition remains untested. Automated Rust native snapshot/consume/sign/HTTP→real Python bridge/runtime/temporary ledger test approves once, rejects and replays with exactly one mock command. Cross-language vectors, cancellation, concurrency and uncertain/restart tests pass. Real camera→physical Pi→hardware acceptance remains unperformed. [Validation](reports/2026-09-06-native-live-backend-validation.md). |
...
| SUP-10 | Contextual fan-sequence escalation | Latest: fitting stopped by user after hybrid forest + 1,024 nearest-normal prototypes per agent reached 3,000/3,000 synthetic anomaly detections, 31/3,000 normal FP on fresh test. Six tests passed; ~1.11 MB data-only candidate, no Pi activation. Prior candidate results follow: 2026-09-06: generated 8,604 synthetic action/context JSONL rows with normal-only fit/calibration, balanced evaluation, protocol validation and four-step demo files. Stored and hash-verified at Pi `/mnt/alice-usb/normal_behavior/fan-demo-20260906-v2/`; two generator tests passed. Workstation candidate trained: 64 trees per agent, normal-only fitting; 0.8% FP and 33.8% anomaly recall on held-out synthetic data. Demo normal/normal/normal/unusual; exported data-only forest with 1.67e-16 parity error. Four tests passed. Not activated on Pi; lab adapter misses many harmful reductions and cooling reference is below contextual fitter minimum. Train on bounded synthetic small-step fan behavior; classify the later abrupt shutdown from sequence plus fresh telemetry, require review for an eligible `ELEVATED`/`HIGH` result, and prove technician rejection sends no command. |
