...
| 080 | Continue Local Anomaly Scoring | Partial | The environmental runtime loads the bounded data-only hybrid Isolation Forest artifact, scores each authenticated fan proposal against current fan/temperature/power context and records model/calibration provenance. On 2026-09-06, authenticated read-only setup checks verified the active Pi thermal process and existing 950,520-byte model with cooling/power profiles. Direct synthetic Pi probes score LOW/LOW/LOW/HIGH; no new signed runtime request was submitted. [Host evidence](reports/2026-09-06-local-agent-model-setup.md). Measured data and authority-mode orchestration remain. |
...
| SUP-13 | Local agent-host model and MCP readiness | Integrated latest upstream `33c9829` without conflicts; user-authorized [PR #8](https://github.com/theoberk25/Alice/pull/8) opened against main. Fresh console checks/build, root pip check, all three MCP identities, Goose/Qwen tool call, 177 biometric tests and real ArcFace pass. Runtime suite: 562 passed/266 subtests; one stale enterprise fixture failure reproduced on unmodified main. Existing biometric OpenCV metadata exception retained. [Integration evidence](reports/2026-09-06-local-agent-main-integration.md). Historical setup: User-authorized setup on 2026-09-06: verified SSH/sudo and Pi thermal/MCP services; installed MCP 1.29.1, PyYAML 6.0.3 and Goose 1.45.0; provisioned private per-identity profiles/tokens and loopback forward. All three MCP reads pass; eight isolated MCP tests and real ArcFace inference pass. Both existing Llama models respond locally. Qwen 7B and its 32K alias are installed; final Goose/Qwen read-only MCP tool use passes. Direct synthetic Pi model probes score LOW/LOW/LOW/HIGH. Plant was already RUNNING at 70% target and was not reset or driven. [Setup guide](guides/local-agent-host.md), [evidence](reports/2026-09-06-local-agent-model-setup.md). |
