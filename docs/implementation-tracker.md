...
| --- | --- | --- | --- |
| 001 | Define Agent Action Request Schema | Partial | The public [action-request schema][action-schema] now strictly discriminates eight-light `set_light_state` requests from integer 0..100 `set_fan_speed` requests for `SERVER-ROOM-FANS`; signed envelopes are verified in [tests][first-light-tests]. Widening to the general request catalog remains coordinated work. |
...
| 035 | Build Physical Sensor Features | Partial | The deployed fan slice reads fan speed, server-room temperature and power from the USB machine-state snapshot immediately before each decision and binds six numeric model inputs to retained assessment evidence. Raw ESP acquisition, sensor authenticity/freshness and real-data ranges remain pending. |
...
| 038 | Save Isolation Forest Model | Done component | The fan training path exports a bounded data-only JSON forest, calibration references and nearest-normal prototypes. No pickle/joblib deserialization is used. Artifact signing/package distribution remains pending. |
| 039 | Load Isolation Forest Model on Boot | Partial | The first-light Pi runtime now loads and validates the bounded data-only fan artifact at boot and fails closed if model/state configuration is incomplete. The deployed model is a synthetic demo candidate, not a signed enterprise model package. |
| 040 | Score Incoming Requests | Partial | The live Pi scores signed `set_fan_speed` requests against the decision-time USB metric snapshot using per-agent hybrid forest/distance references. Other actions retain existing assessment paths; POST_ACTION sensor scoring and general model dispatch remain. |
...
| ID | Task | Status | Evidence and remaining work |
| --- | --- | --- | --- |
| 045 | Combine Policy and Anomaly Results | Partial | The first-light runtime now combines verified fan permission findings with live model assessments: a permitted normal request may ALLOW, while a permitted model anomaly becomes CHALLENGE and cannot execute before human review. Hard policy outcomes retain precedence. General action families and context rounds remain. |
| 046 | Define ALLOW Logic | Partial | A permitted fan request with a usable LOW assessment now auto-ALLOWs and executes through the protected fan-state controller. This is first-light OFFLINE logic, not the general authority-handover implementation. |
...
| 048 | Define HOLD Logic | Done component | A permission-eligible fan request above the candidate's calibrated anomaly threshold produces immutable `CHALLENGE/ANOMALY_REVIEW_REQUIRED`, remains `PENDING/NOT_EXECUTED`, and exports a bounded assessment summary to native review. Live 60→0% proof preserved USB state at 60%. |
...
| 057 | Produce Final Decision | Partial | The first-light OFFLINE runtime now produces authoritative fan ALLOW or anomaly CHALLENGE decisions from verified permission plus model assessment and records them in the ledger. General decision records, REQUEST_CONTEXT and authority handover remain. |
...
| 060 | Record Model Metadata | Partial | Live fan assessments retain model ID/fingerprint, calibration digest, profile/input digests, score, result and reason codes; the Pi review projection exposes a bounded subset. The active demo artifact is data-only but not yet signed as an enterprise model package. |
...
| 069 | Receive Technician Decision | Partial | The Pi accepts native Ed25519 review proofs bound to request, decision, release, authority, epoch and nonce. Fan HOLDs now use the same path; automated approval changes only fan state and rejection sends no command. Live Merek fan disposition remains pending after he pulls the fan-aware console. |
...
| 071 | Execute Approved Action | Partial | Light actions retain the serial controller. Fan actions use a separate atomic USB state controller only after auto-ALLOW or durable one-use technician approval; rejection never writes. Automated fan approval and live light approval passed; a human live fan approval/rejection remains. |
...
| ID | Task | Status | Evidence and remaining work |
| --- | --- | --- | --- |
| 108 | Run End-to-End Normal Request Test | Partial | Live signed cooling-agent requests 30→40→50→60% each produced `ALLOW/PERMITTED_NORMAL_AUTO` and atomically updated the Pi USB state. General ONLINE flow and real sensor actuation remain pending. |
...
| 112 | Run High-Anomaly Hold Test | Partial | A signed power-agent 70→0% sent through enterprise ingress was receipted in Wazuh before unchanged forwarding, cached on Pi USB, and produced HIGH plus `CHALLENGE/ANOMALY_REVIEW_REQUIRED`; review is eligible and USB remained at 70%. Pi ledger REQUEST/ASSESSMENT/DECISION reached Wazuh as sequences 698–700. Human disposition is pending. |
| 113 | Run Technician Approval Test | Partial | Merek completed a real fresh-face approval of a permission HOLD and the physical light turned on. Automated fan HOLD approval updates USB fan state once and rejection sends no command. Generation 5 preserves existing light grants/keys and adds both fan agents. Live model-backed fan request `82d209dc-a4b7-4489-bfc2-9165b6e704bf` is READY/PENDING/NOT_EXECUTED for a human console test after Merek pulls this build. [Validation](reports/2026-09-06-native-live-backend-validation.md). |
...
| SUP-05 | Remote approval proof bound to current authority | 2026-09-06: deployed main/decision_model/technician_review to Pi with Merek console trust (updated to include `Tech-2`, same public key; previous trust backed up before replacement and restart); retained USB/ESP/Wazuh settings and backed up replaced code at `/home/pi/first-light/review-upgrade-20260906`. Local `pytest tests/test_technician_review.py -q`: 64 passed. Human camera→Pi→LED acceptance pending. Implemented for the first-light runtime: exact signed request/decision/release/authority/epoch/nonce/action and fresh session, with replay/concurrency/uncertain-delivery tests. General enterprise authority transfer and real camera/physical-Pi acceptance remain. [Contract](contracts/technician-runtime-review.md). |
...
| SUP-10 | Contextual fan-sequence escalation | Activated first-light slice: a 928 KB data-only hybrid Isolation Forest/nearest-normal candidate scores six decision-time fan/temperature/power features on the 2 GB Pi without sklearn or pickle. Fresh synthetic evaluation detected 3,000/3,000 anomalies with 32/3,000 normal false positives; demo sequence is normal/normal/normal/unusual. Live 30→40→50→60% ALLOW and 60→0% HIGH/CHALLENGE proof passed, with Wazuh receipt and no anomalous execution. Corpus remains at `/mnt/alice-usb/normal_behavior/fan-demo-20260906-v2/`. Real sensor validation, signed model packaging and POST_ACTION scoring remain. |
