...
and `--serial-timeout` (2.0 s, finite and at most 10 s) are available if needed.
...
## Merged regression checks

The integration retains the serial, HTTP, signed SQL snapshot and Wazuh options.
Run the following with pytest installed in your chosen test environment:

```sh
python -m pytest tests/test_first_light_serial.py tests/test_serial_firmware.py tests/test_snapshot_wazuh_integration.py -q
```
 The serial PTY test needs `requirements-hardware.txt`; firmware
