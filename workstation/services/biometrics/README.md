...
Modules separate configuration, schemas, image preprocessing/quality, detector/embedding inference, and encrypted persistence. Tests inject a deterministic engine for error/security paths. `scripts/smoke-arcface.py` separately checks actual CPU inference on a public sample. The app never persists raw face frames. This is identity matching only: liveness and anti-spoof are not implemented.
