...
All events carry `schema_version: "1.0"`, `event_type`, and an ISO UTC `timestamp`. The console's executable schemas live in `packages/contracts/src/alice/events.ts` relative to `workstation/`; generated JSON Schema files in this directory support non-TypeScript teammates. They have not been promoted into `common/` or reconciled with the core contracts. See the [migration assessment](../integration/main-repository-migration.md) for version, anomaly, and ONLINE/OFFLINE authority gaps.
