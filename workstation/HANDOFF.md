...
starts with disposable mock data. See [Main repository migration verification](docs/development/verification.md#main-repository-migration-verification)
...
For the shortest operator path, use [Facial verification quick start](docs/development/facial-verification-quickstart.md).
...
The detailed wire agreement is [docs/integration/upstream-alice.md](docs/integration/upstream-alice.md). Keep that file as the console executable-contract reference under the main repository integration requirements; this subsystem handoff is the wider implementation/backlog map.
...
Detailed evidence: [verification.md](docs/development/verification.md).
...
Useful commands from the `workstation/` subsystem root:
...
ALICE_TEST_OLLAMA_MODEL=llama3.1:8b node scripts/rust.mjs test ollama_live_gateway -- --ignored --nocapture
npm run build:app
```
...
1. [README](README.md): setup, run/test/build commands and initial scope.
2. [Facial verification quick start](docs/development/facial-verification-quickstart.md): current credentials/enrollment state and exact operator steps.
3. [Upstream agreement](docs/integration/upstream-alice.md): wire events/actions and team integration decisions.
4. [Architecture overview](docs/architecture/overview.md), [HOLD workflow](docs/architecture/hold-workflow.md), [LLM boundary](docs/architecture/llm-boundary.md), [biometric boundary](docs/architecture/biometrics.md).
5. [Contract reference](docs/contracts/alice-events.md), [legacy compatibility](docs/contracts/legacy-dashboard-contract.md), [agent/service status](docs/contracts/agent-status.md).
6. [Verification evidence](docs/development/verification.md), [Mac setup](docs/development/mac-setup.md), [mock scenarios](docs/development/mock-scenarios.md), [contribution guide](CONTRIBUTING.md).
...
- Generated/documented contracts: `scripts/generate-contracts.ts`, `docs/contracts/alice-events.schema.json`, `docs/contracts/alice-events.md`.
