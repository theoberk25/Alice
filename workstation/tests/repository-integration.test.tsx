...
const files = ['apps', 'packages', 'services', 'scripts', 'fixtures'].flatMap((directory) =>
  sourceFiles(resolve(workspace, directory)),
);
...
it('keeps renderer, contracts and tooling imports inside the workstation or installed dependencies', () => {
  ...
  for (const { file, name } of imports) {
    ...
    if (result)
      expect(insideWorkspace(realpathSync(result.resolvedFileName)), `${file}: ${name}`).toBe(true);
