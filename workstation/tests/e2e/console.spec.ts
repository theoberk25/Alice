...
test('HOLD → failed face → fresh retry → structured approval', async ({ page }) => {
  ...
  await page.screenshot({ path: 'artifacts/screenshots/operations-desktop.png', fullPage: true });
...
test('mobile layout avoids horizontal overflow', async ({ page }) => {
  ...
  await page.screenshot({ path: 'artifacts/screenshots/operations-mobile.png', fullPage: true });
});
test('automatic HOLD clarification waits for reassessment and approves only DEC-185 with a fresh face check', async ({
  ...
  await page.screenshot({ path: 'artifacts/screenshots/reassessment-desktop.png', fullPage: true });
